...
We then randomly place several cars into the cells and each cell is assigned a numerical value: 
    - -1 -> cell is empty, no car
    - 0-5 -> cell has a car with velocity 0-5
...
1. Car motion: Finally, all cars are moved forward the number of cells equal to their velocity. For example, if the velocity is 3, the car is moved forward 3 cells.
2. Acceleration: All cars not at the maximum velocity have their velocity increased by one unit. For example, if the velocity is 4 it is increased to 5.
